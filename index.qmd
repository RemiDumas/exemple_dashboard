---
title: "Indicateurs de devellopement par continent"
format: 
  dashboard:
    code-tools: true
    theme: yeti
...
```{r}
#| component: valuebox
#| title: Espérance de vie en France en 2007
#| width: 15%
list(
  icon = "heart-fill",
  color = "info",
  value = gapminder %>% filter(year == "2007", country == "France") %>% pull(lifeExp) %>% round(0) %>% paste("ans")
)
```

```{r}
#| component: valuebox
#| title: PIB par français en 2007
#| width: 15%
list(
  icon = "cash-coin",
  color = "secondary",
  value = gapminder %>% filter(year == "2007", country == "France") %>% pull(gdpPercap) %>% round(0) %>% paste("$")
)
```
